...
#
# It installs the NixOS system profile (#default) and, if possible, provisions
# the user's home configuration with standalone home-manager so the first login
# is fully set up.
set -euo pipefail
...
write_local_config() {
  ...
  # hosts/local.nix is the machine-identity half of the `machine` specialArg
  # consumed by both nixosConfigurations and homeConfigurations. The other half
  # (hosts/ghostty.conf) is committed and copied along with the repo.
...
provision_home_config() {
  ...
  nixos-enter --root "${target_root}" -c "su -s /bin/bash ${user} -c 'export PATH=/run/current-system/sw/bin:\$PATH HOME=/home/${user}; home-manager switch --flake /etc/nixos#${user}'"
}
...
main() {
  ...
  enroll_secure_boot_keys
  if provision_home_config "${user}"; then
